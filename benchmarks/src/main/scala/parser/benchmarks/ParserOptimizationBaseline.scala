...
/** Baseline benchmarks for parser implementations BEFORE adding .memoize optimizations.
  *
  * This establishes performance baselines so we can measure the impact of adding .memoize to hot
  * paths like xmlName, whitespace parsers, and number/string literals.
  *
  * Scenarios tested:
...
object ParserOptimizationBaseline {
  ...
  def runBenchmark(
    ...
    // Warmup
    for i <- 0 until warmupIterations do {
      fn()
      if i % 1000 == 0 && i > 0 then {
        print(".")
        System.out.flush()
      }
    }
