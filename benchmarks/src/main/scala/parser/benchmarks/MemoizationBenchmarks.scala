...
object MemoizationBenchmarks {
  ...
  /** Measures pure cache hit performance without parsing overhead.
    *
    * Creates a scenario where the same parser is tried multiple times at the same position through
    * backtracking. The first attempt caches the result, subsequent attempts should hit the cache.
    *
  ...
  def cacheHitPerformance(): Unit = {
    ...
    for _ <- 0 until iterations do memoizeTest.run(input)
    val memoizeTime = (System.nanoTime() - memoizeStart) / 1_000_000

    // Benchmark rule
    val ruleStart = System.nanoTime()
    for _ <- 0 until iterations do ruleTest.run(input)
  ...
  /** Measures performance when backtracking occurs but at different positions.
    *
    * This tests the overhead of memoization infrastructure when cache hits don't occur (different
    * positions), but backtracking happens.
    *
    * Expected: .memoize and rule should be similar since cache misses occur, but .memoize might be
    * slightly faster due to simpler cache structure.
    */
  def backtrackingPerformance(): Unit = {
    ...
    val memoizeParser = (memoizedLetter.many1 ~ char('-') ~ memoizedDigit.many1).map { case ((letters, _), digits) =>
      (letters.mkString, digits.mkString)
    }

    lazy val ruledLetter: Parser[ParseError, Char] = rule(letter)
    lazy val ruledDigit: Parser[ParseError, Char] = rule(digit)
    val ruleParser = (ruledLetter.many1 ~ char('-') ~ ruledDigit.many1).map { case ((letters, _), digits) =>
      (letters.mkString, digits.mkString)
    }
    ...
    // Benchmark .memoize
    val memoizeStart = System.nanoTime()
    for _ <- 0 until iterations do memoizeParser.run(input)
    val memoizeTime = (System.nanoTime() - memoizeStart) / 1_000_000

    // Benchmark rule
    val ruleStart = System.nanoTime()
    for _ <- 0 until iterations do ruleParser.run(input)
    val ruleTime = (System.nanoTime() - ruleStart) / 1_000_000
  ...
  /** Real-world scenario: parsing expressions with memoized components.
    *
    * This tests a more realistic parser where memoization provides benefits by caching expensive
    * sub-parsers that are tried multiple times.
    *
    * Expected: .memoize should show clear benefits over non-memoized, and be faster than rule due
    * to cache hit performance.
    */
  def complexExpressionPerformance(): Unit = {
    ...
    val parserBaseline = (numberBaseline ~ whitespace ~ char('+') ~ whitespace ~ numberBaseline).map {
      case ((((a, _), _), _), b) => a + b
    }

    // Memoized version
    val numberMemoized = digit.many1.map(_.mkString.toInt).memoize
    val parserMemoized = (numberMemoized ~ whitespace ~ char('+') ~ whitespace ~ numberMemoized).map {
      case ((((a, _), _), _), b) => a + b
    }
    ...
    val parserRuled = (numberRuled ~ whitespace ~ char('+') ~ whitespace ~ numberRuled).map {
      case ((((a, _), _), _), b) => a + b
    }
    ...
    for _ <- 0 until iterations do parserBaseline.run(input)
    val baselineTime = (System.nanoTime() - baselineStart) / 1_000_000

    // Benchmark .memoize
    val memoizeStart = System.nanoTime()
    for _ <- 0 until iterations do parserMemoized.run(input)
    val memoizeTime = (System.nanoTime() - memoizeStart) / 1_000_000

    // Benchmark rule
    val ruleStart = System.nanoTime()
    for _ <- 0 until iterations do parserRuled.run(input)
    val ruleTime = (System.nanoTime() - ruleStart) / 1_000_000
  ...
  /** Measures memory allocation patterns.
    *
    * This doesn't directly measure performance but shows allocation overhead. Useful for
  ...
  def memoryAllocationTest(): Unit = {
    ...
    for _ <- 0 until iterations do memoizedParser.run(input)
    ...
    println("  Note: Allocation measurements are approximate due to GC behavior")
  }
