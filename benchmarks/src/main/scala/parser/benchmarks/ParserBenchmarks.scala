...
/** JMH benchmarks for Rumil parser combinators.
  ...
  *
  * Options: -i N : Number of measurement iterations -wi N : Number of warmup iterations -f N :
  * Number of forks -t N : Number of threads
  */
...
class ParserBenchmarks {
  ...
  val catsInt: cats.parse.Parser[Int] = cats.parse.Numbers.digits.map(_.toInt)
  val catsCommaSep: Parser0[List[Int]] = catsInt.repSep0(cats.parse.Parser.char(','))

  val catsStringMatch: cats.parse.Parser[Unit] = cats.parse.Parser.string("hello world")

  val catsChoice10: cats.parse.Parser[Unit] = cats.parse.Parser.oneOf(
    List(
  ...
  val rumilSeq10: Parser[ParseError, Any] = {
    ...
    for _ <- 1 until 10 do p = p ~ satisfy(_.isDigit, "digit")
    p
  }

  val rumilSeq50: Parser[ParseError, Any] = {
    var p: Parser[ParseError, Any] = satisfy(_.isDigit, "digit")
    for _ <- 1 until 50 do p = p ~ satisfy(_.isDigit, "digit")
    p
  }

  val rumilSeq100: Parser[ParseError, Any] = {
    var p: Parser[ParseError, Any] = satisfy(_.isDigit, "digit")
    for _ <- 1 until 100 do p = p ~ satisfy(_.isDigit, "digit")
    p
  }
  ...
  val catsSeq10: cats.parse.Parser[Any] = {
    ...
    for _ <- 1 until 10 do
      p = (p ~ cats.parse.Parser.charWhere(_.isDigit)).map { case (a, b) =>
        (a, b)
      }
    p
  }
