...
extension [E, A](p: Parser[E, A]) {
  ...
  @deprecated("Use orElse instead — after the 0.4.0 flattening they are identical", "0.4.0")
  ...
  inline def skipMany1: Parser[E, Unit] =
    parser.core.skipMany1(p)

  @deprecated("Use skipMany1 instead", "0.4.0")
  inline def skipManyNonEmpty: Parser[E, Unit] =
    parser.core.skipMany1(p)

  /** Captures the input consumed by this parser as a String */
  inline def capture: Parser[E, String] =
    parser.core.capture(p)
  ...
  /** Tries this parser, falling back to another if it fails (fast alternation).
    *
    * This is simple alternation without error tracking. When this parser fails, the fallback is
    * tried. If fallback succeeds, returns Success with no error information from the failed primary
    * parser.
    *
  ...
  /** Tries this parser, falling back to another with error tracking (resilient parsing).
    *
    * When this parser fails, the fallback runs and the result is `Partial` to preserve the original
    * error information. This enables resilient parsing where errors are accumulated but parsing
    * continues.
    *
  ...
  /** Memoizes parser results for improved performance on expensive parsers.
    *
    * Caches parse results by position to avoid redundant work. Unlike `rule`, this does NOT support
    * left-recursion - use `rule` for left-recursive grammars.
    *
    ...
    *
    * @return
    *   A memoized parser with cached results
    */
...
extension [A](p: Parser[ParseError, A]) {
  ...
  @deprecated("Use named instead", "0.4.0")
  inline def label(name: String): Parser[ParseError, A] =
    named(name)

  /** Replaces parser errors with a custom message.
    *
    * When the parser fails, all errors are replaced with a single `ParseError.Custom` containing
