...
/** Type-safe key for memoization table.
  ...
  *
  * IMPORTANT: MemoKey uses reference equality (===), not structural equality (==). Each MemoKey
  * instance is unique - two keys created separately will never match, even if they have the same
  * type parameters. This is intentional: each `rule` call site gets its own unique key for
  * memoization.
  *
  ...
  *
  * @tparam E
  *   The error type of the memoized parser result
  * @tparam A
  *   The value type of the memoized parser result
  */
