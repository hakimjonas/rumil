...
/** Parses a specific character.
  ...
  * @return
  *   A parser that succeeds if the next character is c
  *
  * Example:
  * {{{
...
/** Parses a character that satisfies a predicate.
  *
  * @param pred
  *   Predicate function to test characters
  * @param expected
  *   Description of expected input for error messages
  * @return
  *   A parser that succeeds if the predicate returns true
  *
  * Example:
  * {{{
...
/** Parses any character from a given string.
  ...
  * @return
  *   A parser that succeeds if the next character is in chars
  *
  * Example:
  * {{{
...
/** Parses any character NOT in a given string.
  ...
  * @return
  *   A parser that succeeds if the next character is not in chars
  *
  * Example:
  * {{{
...
/** Parses an exact string.
  ...
  *   The string to match
  * @return
  *   A parser that succeeds if the input starts with s
  *
  * Example:
  * {{{
...
def string(s: String): Parser[ParseError, String] =
  if s.isEmpty then succeed("") else Parser.StringMatch(s)

/** Parses one of several strings using radix tree for O(m) matching.
  *
  * This is more efficient than chaining multiple `string()` parsers with `|` when there are 3+
  * alternatives, as it uses a radix tree internally.
  *
  * @param strings
  *   The strings to match (at least one required)
  * @return
  *   A parser that matches any of the input strings
  *
  * Example:
  * {{{
...
/** Parses one of several keyword-value pairs using radix tree for O(m) matching.
  *
  * Maps each matched keyword to its corresponding value. Uses radix tree internally for efficient
  * matching regardless of the number of keywords.
  *
  * @param mappings
  *   Map of keyword strings to their corresponding values
  * @return
  *   A parser that matches any keyword and returns its mapped value
  *
  * Example:
  * {{{
...
/** Parses zero or more whitespace characters.
  *
  * Always succeeds, consuming => much whitespace => possible.
  *
  * Example:
  * {{{
...
/** Parses a value and consumes trailing whitespace.
  ...
  *   The parser to run
  * @return
  *   A parser that runs p then consumes trailing whitespace
  *
  * Example:
  * {{{
...
/** Parses a symbol (string with trailing whitespace consumed).
  ...
  *   The symbol to parse
  * @return
  *   A parser that matches s and consumes trailing whitespace
  *
  * Example:
  * {{{
...
/** Defers parser construction until needed.
  *
  * Essential for defining recursive parsers without stack overflow. The thunk is evaluated lazily
  * each time the parser is used.
  *
  * Example:
  * {{{
  ...
  *
  * @param p
  *   The parser to defer (by-name parameter)
  * @return
  *   A parser that lazily evaluates p
  */
...
/** Creates a memoized parser rule that supports left recursion.
  ...
  *
  * ⚠️ **PERFORMANCE NOTE** ⚠️
  *
  * `rule` measures ~1.4–2.5x slower than the equivalent unmemoized grammar on a 2,000-term additive
  * chain (measured 2026-08-31, scala-cli harness). ONLY use it when you need:
  ...
  *
  * @param p
  *   The parser to memoize (by-name parameter)
  * @return
  *   A memoized parser with left recursion support
  */
...
/** Memoizes a parser for improved performance on expensive parsers.
  *
  * Caches parse results by position to avoid redundant work. Unlike `rule`, this uses simple
  * caching without left-recursion support for better performance.
  *
  ...
  * @return
  *   A memoized parser with simple caching
  */
