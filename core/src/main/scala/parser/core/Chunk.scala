...
final class Chunk[+A] private[parser] (
  ...
  def apply(i: Int): A = {
    ...
    array(i).asInstanceOf[A] // scalafix:ok DisableSyntax.asInstanceOf
  }
  ...
  def toList: List[A] = {
    ...
    var i = length - 1
    while i >= 0 do {
      result = array(i).asInstanceOf[A] :: result // scalafix:ok DisableSyntax.asInstanceOf
      i -= 1
    }
    result
  }
  ...
  def toArray[A1 >: A: scala.reflect.ClassTag]: Array[A1] = {
    ...
    var i = 0
    while i < length do {
      result(i) = array(i).asInstanceOf[A1] // scalafix:ok DisableSyntax.asInstanceOf
      i += 1
    }
    result
  }
  ...
  def iterator: Iterator[A] = new AbstractIterator[A] {
    ...
    def next(): A = {
      ...
      val result = array(i).asInstanceOf[A] // scalafix:ok DisableSyntax.asInstanceOf
      i += 1
      result
    }
  }
  ...
  def map[B](f: A => B): Chunk[B] = {
    ...
    var i = 0
    while i < length do {
      builder += f(array(i).asInstanceOf[A]) // scalafix:ok DisableSyntax.asInstanceOf
      i += 1
    }
    builder.result()
  }
  ...
  def filter(pred: A => Boolean): Chunk[A] = {
    ...
    var i = 0
    while i < length do {
      val elem = array(i).asInstanceOf[A] // scalafix:ok DisableSyntax.asInstanceOf
      if pred(elem) then {
        builder += elem
      }
      i += 1
  ...
  def headOption: Option[A] =
    if length > 0 then Some(array(0).asInstanceOf[A]) // scalafix:ok DisableSyntax.asInstanceOf
    else None
