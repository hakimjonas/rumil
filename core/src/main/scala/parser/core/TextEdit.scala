...
/** Represents a text edit operation on source code.
  *
  * A TextEdit describes a range of text to delete and new text to insert in its place. This is the
  * fundamental unit for incremental parsing.
  *
  * Examples:
  ...
  *
  * @param startOffset
  *   The start of the range to replace (inclusive, 0-indexed)
  * @param endOffset
  *   The end of the range to replace (exclusive, 0-indexed)
  * @param newText
...
final case class TextEdit(
  ...
  /** The net change in document length after this edit. Positive means the document grows, negative
    * means it shrinks.
    */
  ...
  /** Apply this edit to source text, producing new source text.
    ...
    * @return
    *   The source text with this edit applied
    */
  def apply(source: String): String = {
    require(endOffset <= source.length, s"endOffset ($endOffset) exceeds source length (${source.length})")
  ...
  /** Check if this edit affects a given offset range.
    ...
    * @return
    *   true if this edit overlaps with or is adjacent to the range
    */
  ...
  /** Adjust an offset to account for this edit.
    *
    * Offsets before the edit are unchanged. Offsets within the deleted range map to the start of
    * the edit. Offsets after the edit are shifted by the length delta.
    *
    * @param offset
    *   The offset to adjust
    * @return
    *   The adjusted offset in the post-edit document
    */
  def adjustOffset(offset: Int): Int =
    if offset <= startOffset then offset
    else if offset < endOffset then startOffset
    else offset + lengthDelta

  override def toString: String =
    if isInsertion then s"TextEdit.insert($startOffset, ${newText.take(20).repr})"
    else if isDeletion then s"TextEdit.delete($startOffset, $endOffset)"
...
object TextEdit {
  ...
  /** Compose multiple edits into a sequence that can be applied left-to-right.
    *
    * Edits must be non-overlapping and sorted by offset. Returns edits adjusted so they can be
    * applied sequentially.
    *
    * @param edits
    *   The edits to compose (must be sorted by startOffset)
    * @return
    *   Adjusted edits that can be applied in order
    */
