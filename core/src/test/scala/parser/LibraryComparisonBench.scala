...
/** Fair comparison benchmark: Rumil vs cats-parse vs zio-parser.
  *
  * Tests 5 common parsing workloads to give realistic performance comparison. All parsers built
  * once (excludes construction overhead).
  */
class LibraryComparisonBench extends munit.FunSuite {
  ...
  val manyInput: String = "a" * 1000
  val seqInput: String = "1" * 100
  val numberInput = "42"

  // Rumil parsers
  val rumilString: parser.core.Parser[parser.core.ParseError, String] = {
    import parser.core.*
    string("hello")
  }
  val rumilChoice: parser.core.Parser[parser.core.ParseError, String] = {
    import parser.core.*
  ...
  val rumilMany: parser.core.Parser[parser.core.ParseError, List[Char]] = {
    import parser.core.*
    parser.core.many(char('a'))
  }
  val rumilSeq: parser.core.Parser[parser.core.ParseError, Any] = {
    import parser.core.*
    import parser.syntax.*
  ...
  val rumilNumber: parser.core.Parser[parser.core.ParseError, Int] = {
    import parser.core.*
    import parser.syntax.*
  ...
  // cats-parse parsers
  val catsString: cats.parse.Parser[String] = {
    import cats.parse.{Parser as P}
    P.string("hello").string
  }
  val catsChoice: cats.parse.Parser[String] = {
    import cats.parse.{Parser as P}
  ...
  val catsMany: cats.parse.Parser[List[Char]] = {
    import cats.parse.{Parser as P}
    P.charIn('a').rep.map(_.toList.map(_.toString.head))
  }
  val catsSeq: cats.parse.Parser[Any] = {
    import cats.parse.{Parser as P}
  ...
  val catsNumber: cats.parse.Parser[Int] = {
    import cats.parse.Numbers
    Numbers.digits.map(_.toInt)
  }

  // zio-parser parsers
  val zioString: zio.parser.Syntax[String, Char, Char, String] = {
    import zio.parser.*
    Syntax.string("hello", "hello")
  }
  val zioChoice: zio.parser.Syntax[String, Char, Char, String] = {
    import zio.parser.*
  ...
  val zioMany: zio.parser.Syntax[String, Char, Char, zio.Chunk[Unit]] = {
    import zio.parser.*
    Syntax.char('a').repeat
  }
  val zioNumber: zio.parser.Syntax[String, Char, Char, Int] = {
    import zio.parser.*
  ...
  assert(
    parser.runtime
      ...
      .isInstanceOf[parser.core.Result.Success[?, ?]], // scalafix:ok DisableSyntax.isInstanceOf
    "Rumil string"
  )
  assert(
    parser.runtime
