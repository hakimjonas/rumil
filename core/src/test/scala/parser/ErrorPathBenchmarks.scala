 ...
 *
 * These complement the happy-path benchmarks in ComprehensiveLibraryComparison
 * by testing scenarios where error accumulation matters.
 *
 * Categories:
...
class ErrorPathBenchmarks extends munit.FunSuite {
  ...
  test("error path 3.1: Choice exhaustion (10 alternatives, all fail, 10K iterations)") {
    ...
    assert(parser.runtime.run(rumilParser, input).isInstanceOf[parser.core.Result.Failure[?, ?]])
