...
/** Comparative benchmarks between Rumil and cats-parse.
  *
  * These are simple timing comparisons for common parsing patterns. For rigorous benchmarks, use
  * JMH instead.
  */
class CatsParseComparison extends FunSuite {
  ...
  def timed[A](label: String, iterations: Int)(f: => A): (A, Long) = {
    ...
    var result: A = null.asInstanceOf[A] // scalafix:ok DisableSyntax.null, DisableSyntax.asInstanceOf
