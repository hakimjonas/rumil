...
| `many1(p)` / `p.many1` | `Parser[E, List[A]]` | One or more |
| `optional(p)` / `p.optional` | `Parser[E, Option[A]]` | Zero or one |
| `count(n, p)` / `p.count(n)` | `Parser[E, List[A]]` | Exactly n times |
| `manyAtLeast(n)(p)` / `p.manyAtLeast(n)` | `Parser[E, List[A]]` | At least n times |
| `skipMany(p)` / `p.skipMany` | `Parser[E, Unit]` | Zero or more, discard results |
| `skipMany1(p)` / `p.skipMany1` | `Parser[E, Unit]` | One or more, discard results |

### Separated/Terminated

| Function | Signature | Description |
|----------|-----------|-------------|
| `sepBy(p, sep)` / `p.sepBy(sep)` | `Parser[E, List[A]]` | Zero+ separated by sep |
| `sepBy1(p, sep)` / `p.sepBy1(sep)` | `Parser[E, List[A]]` | One+ separated by sep |
| `endBy(p, end)` / `p.endBy(end)` | `Parser[E, List[A]]` | Zero+ terminated by end |

**Deprecated aliases** (deprecated in 0.4.0, removal scheduled for a future major):

| Deprecated | Use instead |
|------------|-------------|
| `manyNonEmpty` | `many1` |
| `times(n, p)` | `count(n, p)` |
| `skipManyNonEmpty` | `skipMany1` |
| `separatedBy(sep)` | `sepBy(sep)` |
| `separatedByNonEmpty(sep)` | `sepBy1(sep)` |
| `endedBy(end)` | `endBy(end)` |
| `chainLeft1(op)` | `chainl1(op)` |
| `chainRight1(op)` | `chainr1(op)` |
| `label(name)` | `named(name)` |

**Examples:**

```scala
digit.many.run("123x")                // Success(List('1','2','3'), 3)
digit.many1.run("x")                  // Failure
digit.optional.run("x")               // Success(None, 0)
digit.count(3).run("123")             // Success(List('1','2','3'), 3)
digit.sepBy(char(',')).run("1,2,3")   // Success(List('1','2','3'), 5)
```
...
| Function | Signature | Description |
|----------|-----------|-------------|
| `orElse(p, fallback)` / `p.orElse(fallback)` / `p \| fallback` | `Parser[E, A]` | Fast alternation: try p, if fails try fallback (no error tracking) |
| `choice(parsers)` | `List[Parser[E,A]] => Parser[E,A]` | Try parsers in order (flattens; radix `StringChoice` for pure string alternatives) |
| `firstCharChoice(dispatch, fallback)` | `Parser[E, A]` | O(1) first-character dispatch |

**Important:** alternation backtracks on failure — if a branch consumes input then fails, position is restored before the next branch. For error-tracking alternation use `recover` (see Error Handling below).
...
```scala
val parser = for {
  name <- letter.many1.map(_.mkString)
  _    <- char('=')
  value <- digit.many1.map(_.mkString.toInt)
} yield (name, value)
...
### Alternation vs Recovery

Two combinators try a fallback parser when the primary fails. They differ only in error
tracking:

| Function | Behavior on Failure | Returns |
|----------|---------------------|---------|
| `orElse(p, fallback)` / `p.orElse(fallback)` | Fast alternation, errors discarded | `Success` (or `Partial` from the fallback itself) |
| `recover(p, fallback)` / `p.recover(fallback)` | Alternation with error tracking | `Partial` with the primary's errors when the fallback succeeds |
...
```scala
// orElse: fast alternation — errors from failed branches are not tracked
orElse(char('a'), char('b')).run("b")   // Success('b', 1)
orElse(digit, succeed('0')).run("x")    // Success('0', 0)

// recover: same fallback, but the primary's errors are preserved
recover(digit, succeed('0')).run("x")   // Partial('0', errors, 0)

// recover (function form): always succeeds with default
recover(digit)(_ => '0').run("x")       // Success('0', 0)

// recoverWith: try alternative parser built from the error
recoverWith(digit)(_ => char('?')).run("?")  // Success('?', 1)
```

Use `orElse` for alternation between valid options (the common case — it matches cats-parse /
Parsec `<|>` semantics). Use `recover` when diagnostics from the failed primary matter (IDE
squiggles, resilient editors). Measured delta: `orElse` is faster than `recover` on
fallback-heavy workloads (2025-11 measurement).
...
```scala
expect(digit.many1, "number required").run("x")
...
**Performance note:** measured ~1.4–2.5x slower than the equivalent unmemoized grammar (2026-08-31 measurement) — use `rule` only for left-recursive grammars.
...
### Built-in Decoders (sarati's JsonDecoders)

The decoder layer for rumil's JSON AST is sarati's — import `net.ghoula.sarati.codec.JsonDecoders.given` for:
...
(`parser.interop.JsonDecoders` was removed with the interop deprecation; sarati is the maintained successor.)
...
- [Performance](./performance.md) - Optimization overview and benchmark sources
