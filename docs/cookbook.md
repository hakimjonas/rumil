...
**Key techniques:**
- Keyword matching with flexible surrounding whitespace
...
`|` and `.orElse` are fast alternation — failed branches leave no error tracking behind. When
recovered input must carry diagnostics, use `.recover`: a successful fallback then returns
`Partial` with the primary's errors.

```scala
val tracked = mostSpecific.recover(lessSpecific).recover(fallback)
```
...
- **Error handling**: Add `.named()` for better error messages; use `.recover` (tracked) vs
  `.orElse` (fast) deliberately — see the [Error Handling Guide](./error-handling.md)
- **Performance**: See [Performance](./performance.md) for the optimization overview
...
**Questions or suggestions?** Open an issue at [github.com/hakimjonas/rumil](https://github.com/hakimjonas/rumil/issues)!
