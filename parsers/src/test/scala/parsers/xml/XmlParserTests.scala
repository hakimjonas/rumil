...
class XmlParserTests extends FunSuite {
  ...
  test("fail on mismatched tags") {
    ...
    assert(
      messages.exists(m => m.contains("mismatched end tag") && m.contains("root") && m.contains("other")),
      s"unexpected errors: ${result.errors}"
    )
    // the mismatch is positioned: offset 6 is where </other> starts
    val located = result.errors.collect {
      case ParseError.Custom(m, loc) if m.contains("mismatched end tag") => loc
    }
    assert(located.nonEmpty, s"expected a positioned mismatched-tag error, got ${result.errors}")
    assert(located.forall(_.offset >= 6), s"mismatched-tag error not positioned at the close tag: $located")
  }

  test("fail on mismatched nested tags") {
    val xml = "<root><a><b></c></b></root>"
    val result = parseXmlFragment(xml)
    assert(result.isFailure, s"expected failure, got $result")
  }

  test("mismatched tag does not swallow siblings as text") {
    val xml = "<root>text</wrong></root>"
    val result = parseXmlFragment(xml)
    assert(result.isFailure, s"expected failure, got $result")
  }
