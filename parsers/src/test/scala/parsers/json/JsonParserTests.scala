...
class JsonParserTests extends FunSuite {
  ...
  test("parse integer zero") {
    ...
    assertEquals(result.toOption, Some(Number(0.0, Some("0"))))
  }

  test("parse positive integer") {
    val result = parseJson("42")
    assertEquals(result.toOption, Some(Number(42.0, Some("42"))))
  }

  test("parse negative integer") {
    val result = parseJson("-17")
    assertEquals(result.toOption, Some(Number(-17.0, Some("-17"))))
  }

  test("parse decimal number") {
    val result = parseJson("3.14159")
    assertEquals(result.toOption, Some(Number(3.14159, Some("3.14159"))))
  }

  test("parse negative decimal") {
    val result = parseJson("-2.5")
    assertEquals(result.toOption, Some(Number(-2.5, Some("-2.5"))))
  }

  test("parse number with exponent") {
    val result = parseJson("1e10")
    // the raw token is preserved: "1e10" is a distinct spelling of 1.0E10
    assertEquals(result.toOption, Some(Number(1e10, Some("1e10"))))
  }

  test("parse number with positive exponent") {
    val result = parseJson("1.5e+3")
    assertEquals(result.toOption, Some(Number(1500.0, Some("1.5e+3"))))
  }

  test("parse number with negative exponent") {
    val result = parseJson("2e-3")
    assertEquals(result.toOption, Some(Number(0.002, Some("2e-3"))))
  }

  test("raw token survives spellings the canonical form would change") {
    // Number(1.0) constructed programmatically has raw = None and renders "1" —
    // the parsed AST must keep the original "1.0" spelling (issue: raw token preservation)
    val result = parseJson("""{"n": 1.0}""")
    assertEquals(result.toOption, Some(Object(Map("n" -> Number(1.0, Some("1.0"))))))
  }
  ...
  test("parse array with one element") {
    ...
    assertEquals(result.toOption, Some(Array(List(Number(1, Some("1"))))))
  }

  test("parse array with multiple elements") {
    val result = parseJson("[1,2,3]")
    assertEquals(result.toOption, Some(Array(List(Number(1, Some("1")), Number(2, Some("2")), Number(3, Some("3"))))))
  }
  ...
  test("parse nested arrays") {
    ...
    assertEquals(
      ...
      Some(
        Array(
          List(
            Array(List(Number(1, Some("1")), Number(2, Some("2")))),
            Array(List(Number(3, Some("3")), Number(4, Some("4"))))
          )
        )
      )
    )
  ...
  test("parse object with array value") {
    ...
    assertEquals(
      ...
      Some(
        Object(
          Map(
            "numbers" -> Array(List(Number(1, Some("1")), Number(2, Some("2")), Number(3, Some("3"))))
          )
        )
      )
    )
  ...
  test("RFC 8259: whitespace is allowed before and after values") {
    val result = parseJson("  \n\t 42 \n\t  ")
    assertEquals(result.toOption, Some(Number(42, Some("42"))))
  }
  ...
  test("parse person object") {
    ...
    result.toOption.get match {
      case Object(fields) =>
        ...
        assertEquals(fields("age"), Number(30, Some("30")))
  ...
  test("property: integers round-trip") {
    val prop = forAll(Gen.choose(-1000, 1000)) { n =>
      ...
      // the raw token is preserved: Some(input) — same spelling as the source text
      parseJson(input).toOption.contains(Number(n.toDouble, Some(input)))
    }
    prop.check()
  }
  ...
  test("property: format then parse is value-identity") {
    // numbers carry their raw token: format re-spells the number, so the parsed AST has a
    // fresh raw — the identity this property pins is value-level (raws stripped both sides)
    def stripRaw(v: JsonValue): JsonValue = v match {
      case Number(n, _) => Number(n)
      case Array(elements) => Array(elements.map(stripRaw))
      case Object(fields) => Object(fields.view.mapValues(stripRaw).toMap)
      case other => other
    }
    ...
    val valueGen: Gen[JsonValue] = Gen.oneOf(
      ...
      Gen.choose(-100.0, 100.0).map(n => Number(n)),
    ...
    val prop = forAll(valueGen) { value =>
      ...
      parsed.toOption.map(stripRaw).contains(stripRaw(value))
    }
    prop.check()
  }
}
