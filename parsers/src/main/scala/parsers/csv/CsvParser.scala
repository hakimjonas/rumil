...
/** Parses a CSV document with the given configuration.
  ...
  * @return
  *   Parser that produces a CSV document
  */
...
/** Parses CSV from a string.
  ...
  *   CSV parsing configuration
  * @return
  *   Result containing parsed CSV or errors
  */
...
private def csvField(config: CsvConfig): Parser[ParseError, String] = {
  ...
  (quoted | unquoted).map { field =>
    if config.trimWhitespace then field.trim else field
  }
}
...
/** Parses CSV and validates that all rows have the same number of columns.
  ...
  *   CSV parsing configuration
  * @return
  *   Parser that produces validated CSV with consistent columns
  */
...
def parseCsvWithHeaders(
  input: String,
  config: CsvConfig = defaultCsvConfig
): Result[ParseError, (List[String], CsvDocument)] =
  parseCsv(input, config) match {
    case Result.Success(records, consumed) =>
      records match {
...
/** Parses CSV into a list of maps (each row => a map of header -> value).
  *
  * Useful for working with CSV => records with named fields.
  */
