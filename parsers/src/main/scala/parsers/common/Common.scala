...
/** Parses a hexadecimal digit (0-9, a-f, A-F).
  */
val hexDigit: Parser[ParseError, Char] =
...
/** Parses a Unicode escape sequence (\uXXXX).
  ...
  */
val unicodeEscape: Parser[ParseError, Char] =
  for {
    _ <- char('\\')
    _ <- char('u')
    d1 <- hexDigit
...
/** Parses an optional sign (+ or -), returning 1 or -1.
  */
val sign: Parser[ParseError, Int] =
  (char('+').as(1) | char('-').as(-1)).optional.map(_.getOrElse(1))

/** Parses a signed integer.
  */
val signedInt: Parser[ParseError, Int] =
  for {
    s <- sign
    digits <- digit.many1
