...
enum Parser[+E, +A] {
  ...
  case LookAhead[E, A](parser: Parser[E, A])
    extends Parser[E, A]
  case NotFollowedBy[A](parser: Parser[ParseError, A])
    extends Parser[ParseError, Unit]
  case Named[A](parser: Parser[ParseError, A], name: String)
    extends Parser[ParseError, A]
  case Custom[E, A](run: parser.runtime.ParserState => Result[E, A])
    extends Parser[E, A]
}
