...
import net.ghoula.sarati.codec.FieldTransformer

import scala.quoted.{Expr, Quotes, Type}

/** Example implementation: Creating a FieldTransformer by reading field annotations.
  *
  * This demonstrates how to use Scala 3 inline metaprogramming (quotes reflection API) to read
  * annotations from a case class and generate a customized FieldTransformer.
  *
  * IMPORTANT: This is example code showing the technique. Users can adapt this pattern to implement
  * their own annotation-based transformers.
  *
  * Example usage:
  * {{{
...
object AnnotationFieldTransformer {
  ...
  /** Macro implementation using Scala 3 quotes reflection API.
    ...
    *
    * Note: We use primaryConstructor.paramSymss instead of caseFields because caseFields symbols
    * have empty annotations in Scala 3 (known limitation).
    */
  def deriveImpl[T: Type](using q: Quotes): Expr[FieldTransformer] = {
    ...
    val renameMappings: Map[String, String] = fields.flatMap { field =>
      ...
      if field.hasAnnotation(renameSymbol) then {
