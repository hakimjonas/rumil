 ...
 *
 * IMPORTANT: These annotations are examples, not part of the core Rumil API.
 * They demonstrate how to implement field customization using Scala 3 macros.
 *
 ...
 *
 * Maps a Scala field name to a different name in the source data.
 * Commonly used for converting between naming conventions (snake_case ↔ camelCase).
 *
 * Example:
 * {{{
 * case class Person(
 ...
 *
 * Fields annotated with @Ignore are skipped during decoding.
 ...
 *
 * The decoder tries each name in order until one is found.
 * Useful for handling multiple API versions or legacy field names.
 *
 * Example:
 * {{{
 * case class User(
