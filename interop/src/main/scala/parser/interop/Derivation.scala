...
/** Automatic parser derivation for case classes using Scala 3 inline metaprogramming.
  *
  * @deprecated
  *   The `interop` module is a pre-Sarati twin of the codec layer. Use `net.ghoula:sarati` instead
  *   — its `Decoder`/`Encoder` derivations are the maintained successors (trampoline stack safety,
  *   typed derivation, mandatory `FieldTransformer`). This module stays functional until the 1.0
  *   removal.
  *
  * This object provides compile-time derivation of parsers for product types (case classes). It
  * uses Scala 3 inline metaprogramming (quotes and splices) with Mirror metadata to generate
  * efficient parsers that parse case classes in the format: `ClassName(field1,field2,field3)`
  *
  * Example:
  * {{{
  ...
  * @since 0.2.0
  */
@deprecated("The interop module is superseded by sarati codec derivation; removal in 1.0", "0.4.0")
object Parser {

  /** Derives a parser for a case class automatically.
    *
    * This inline method expands at compile time (quotes and splices) to generate a parser for the
    * given type A. The type must be a product type (case class) and parsers must be available for
    * all field types.
    *
    * @tparam A
    *   The case class type to derive a parser for
    * @return
    *   A parser that can parse the case class from input
    */
  ...
  /** Macro implementation for parser derivation.
    ...
    *
    * @param m
    *   Expression representing the Mirror instance
    * @tparam A
    *   The product type being derived
    * @return
    *   Expression representing the generated parser
    */
  ...
  /** Generates the parser code that combines all field parsers.
    ...
    * @param fieldLabels
    *   The names of the fields (for error messages)
    * @param fieldParsers
    *   The parsers for each field
    * @param mirror
    *   The Mirror instance for reconstruction
    * @return
    *   Expression representing the complete parser
    */
